...
class SplitTest
{
    ...
    public function initiate(Request $request, $test)
    {
        ob_start();


        if ($request->session()->has(config('laravel-ab.cache_key'))) {
            $choices = $request->session()->get(config('laravel-ab.cache_key'));
        } else {
            // find out the different choices, put a random into the session.
            $uid = md5(uniqid().$this->request->getClientIp());
            self::request->cookie(config('laravel-ab.cache_key'), $uid);
            Session::put(config('laravel-ab.cache_key'), $uid);
        }
    ...
    public static function tearDown(Request $request, $test)
    {
        $choice = Session::get(config('laravel-ab.cache_key'));
